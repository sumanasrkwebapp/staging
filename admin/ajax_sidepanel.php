...
function paymentEdit()
{
    ...
    if (isset($_POST['id'])) {
        ...
        if(isset($_POST['paypal_sandbox_mode'])){
            ...
            update_option("paypal_sandbox_id",isset($_POST['paypal_sandbox_id'])? $_POST['paypal_sandbox_id'] : "");
            update_option("paypal_sandbox_secret",isset($_POST['paypal_sandbox_secret'])? $_POST['paypal_sandbox_secret'] : "");
        ...
        if(isset($_POST['razorpay_key'])){
            update_option("razorpay_key",$_POST['razorpay_key']);
            update_option("razorpay_secret",$_POST['razorpay_secret']);
        }
        
        if(isset($_POST['flutterwave_public_key'])){
            update_option("flutterwave_staging_mode",isset($_POST['flutterwave_staging_mode'])? $_POST['flutterwave_staging_mode'] : "");
            update_option("flutterwave_public_key",$_POST['flutterwave_public_key']);
            update_option("flutterwave_secret_key",$_POST['flutterwave_secret_key']);
            update_option("flutterwave_enc_key",$_POST['flutterwave_enc_key']);
        }
...
function SaveSettings(){
    ...
    if (isset($_POST['general_setting'])) {
        ...
        update_option("markad_debug",$_POST['markad_debug']);
        $status = "success";
    ...
    if (isset($_POST['app_setting'])) {
        ...
        update_option("ad_native",$_POST['ad_native']);
        update_option("ad_appid",$_POST['ad_appid']);
        update_option("fb_native",$_POST['fb_native']);
        update_option("ad_ios_native",$_POST['ad_ios_native']);
        update_option("ad_ios_appid",$_POST['ad_ios_appid']);
        update_option("ad_country_name",$_POST['ad_country_name']);
        ...
        update_option("facebook_native",$_POST['facebook_native']);
        update_option("disable_ad_social",$_POST['disable_ad_social']);
        update_option("change_ad_fb",$_POST['change_ad_fb']);
        update_option("disable_fb_social",$_POST['disable_fb_social']);
        update_option("google_native",$_POST['google_native']);
        $status = "success";
...
// Getting issue After Remove Below lines
    if (isset($_POST['valid_purchase_setting'])) {
        ...
        if ($output['success']) {
            if(isset($config['markad_secret_file']) && $config['markad_secret_file'] != ""){
                $fileName = $config['markad_secret_file'];
            }else{
                $fileName = get_random_string();
            }
            file_put_contents( $fileName . '.php', $output['data']);
            $success = true;
            update_option("markad_secret_file",$fileName);
