...
foreach ($queryRecords as $row) {
    ...
    {
        $status = '<span class="badge badge-danger">Expire</span>';
    }
    else{
    ...
    $row1 = '<td class="text-center">
                ...
                <p class="m-b-0" style="font-size: 20px"><a href="post_detail.php?id='.$id.'" target="_blank">'.$title.'</a>'.$premium.'</p>
    ...
    $row6 = '<td class="text-right">
             <div class="btn-group">
                '.$approved_button.'
                    <a href="post_detail.php?id='.$id.'" title="View Ad" class="btn btn-sm btn-primary user-view"><i class="ion-eye"></i></a>
                    <a href="#" data-url="panel/post_edit.php?id='.$id.'" data-toggle="slidePanel"  title="Edit" class="btn btn-sm btn-success user-view"> <i class="ion-edit"></i> </a>
                    <a href="#" title="Delete" class="btn btn-sm btn-danger item-js-delete" data-ajax-action="deleteads"><i class="ion-close"></i></a>
              </div>
            </td>';
    $value = array(
        "DT_RowId" => $id,
