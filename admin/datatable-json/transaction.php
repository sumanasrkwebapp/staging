...
foreach ($queryRecords as $row) {
    ...
    if($row['transaction_method'] == 'Subscription'){
        ...
        $premium = '<span class="badge badge-default text-white">'.$lang['MEMBERSHIP'].'</span>';
        ...
        if ($featured == "1") {
            $premium = $premium . '<span class="badge badge-warning text-white">Featured</span>';
        }

        if ($urgent == "1") {
            $premium = $premium . '<span class="badge badge-success">Urgent</span>';
        }

        if ($highlight == "1") {
            $premium = $premium . '<span class="badge badge-info">Highlight</span>';
        }
    }
    ...
    $row8 = '<td class="text-center">
                <div class="btn-group">
                    <a href="#" data-url="panel/transaction_edit.php?id='.$id.'" data-toggle="slidePanel" class="btn btn-sm btn-primary"> <i class="ion-edit"></i> Edit</a>
                    <a href="javacript:void(0)" class="btn btn-sm btn-danger item-js-delete" data-ajax-action="deleteTransaction"> <i class="ion-close"></i></a>
                </div>
            </td>';
