...
if (isset($_REQUEST['action'])){
    ...
    if ($_REQUEST['action'] == "related_product_ads") { related_product_ads(); }
    ...
    if ($_REQUEST['action'] == "upload_product_video") { upload_product_video(); }
    ...
    if ($_REQUEST['action'] == "searchCity") {searchCity();}
    if ($_REQUEST['action'] == "userTransactions"){userTransactions();}
    if ($_REQUEST['action'] == "ad_custom_data"){ad_custom_data();}
    if ($_REQUEST['action'] == "update_post"){update_post();}
    if ($_REQUEST['action'] == "add_post_like"){add_post_like();}
    if ($_REQUEST['action'] == "add_post_comment"){add_post_comment();}
    if ($_REQUEST['action'] == "add_post_reply"){add_post_reply();}
    if ($_REQUEST['action'] == "get_comment_list"){get_comment_list();}
    if ($_REQUEST['action'] == "get_ads_options"){get_ads_options();}
}
...
function app_config(){
    ...
    $results['ad_native'] = $config['ad_native'];
    $results['ad_appid'] = $config['ad_appid'];
    $results['fb_native'] = $config['fb_native'];
    $results['ad_ios_native'] = $config['ad_ios_native'];
    $results['ad_ios_appid'] = $config['ad_ios_appid'];
    $results['ad_country_name'] = $config['ad_country_name'];
    ...
    $results['disable_ad_social'] = ($config['disable_ad_social'] == 1) ? true : false;
    $results['disable_fb_social'] = ($config['disable_fb_social'] == 1) ? true : false;
    $results['change_ad_fb'] = ($config['change_ad_fb'] == 1) ? true : false;
...
function payment_api_detail_config()
{
    ...
    /******************RAZORPAY*******************/
    $results['razorpay_key'] = $config['razorpay_key'];
    
    /******************FLUTTERWAVE*******************/
    $results['flutterwave_public_key'] = $config['flutterwave_public_key'];
    $results['flutterwave_enc_key'] = $config['flutterwave_enc_key'];
    $results['flutterwave_staging_mode'] = $config['flutterwave_staging_mode'];
    ...
    $results['payumoney_merchant_salt'] = $config['payumoney_merchant_salt'];
...
function login(){
    global $config,$lang,$status,$message,$results;
     ...
     {
        $loggedin = userlogin($_REQUEST['email'], $_REQUEST['password']);
     }
...
function get_products_data($userid=null,$cat_id=null,$subcat_id=null,$location=false,$country_code=null,$state_code=null,$city=null,$status=null,$premium=false,$page=null,$limit=null,$order=false,$sort="id",$sort_order="DESC",$item_id=null){
    global $config,$con,$lang,$results;
    $where = '';
    if($userid != null){
        if($where == '')
   ...
   if($order){
    ...
    $pdo = ORM::get_db();
if($item_id != null){
    $query = "SELECT p.id,p.product_name,p.featured,p.urgent,p.highlight,p.price,p.category,p.sub_category,p.tag,p.screen_shot,p.media_type,p.post_video,p.user_id,p.city,p.country,p.state,p.status,p.hide,p.created_at,p.expire_date,p.description,p.phone
...
{
    $query = "SELECT p.id,p.product_name,p.featured,p.urgent,p.highlight,p.price,p.category,p.sub_category,p.tag,p.screen_shot,p.media_type,p.post_video,p.user_id,p.city,p.country,p.state,p.status,p.hide,p.created_at,p.expire_date,p.description,p.phone,p.latlong,
...
$where ORDER BY $order_by $pagelimit";
    ...
    if ($rows > 0) {
        foreach($result as $info) {
            ...
            $item['cityid'] = $info['city'];
            $item['stateid'] = $info['state'];
            $item['countryid'] = $info['country'];
            $item['latlong'] = $info['latlong'];
...
//             }
            ...
            if($item['media_type'] == 'video'){
                $item['video'] = $config['site_url']."storage/products/".$info['post_video'];
            }
...
function related_product_ads(){
    ...
    $results = get_products_data($user_id,$cat_id,$subcat_id,$location,$country_code,$state_code,$city,$status,$premium,$page,$limit,$order=false,$sort="id",$sort_order="DESC",$item_id);
...
function ad_detail(){
    ...
    if(isset($_REQUEST['item_id'])){
        ...
        if ($num_rows > 0) {
            ...
            $item['seller_createdat'] = $info2['created_at'];
            ...
            $item['media_type'] = $info['media_type'];
            $item['item_video'] = $info['post_video'];
            $item['tag'] = $info['tag'];
            ...
            $item['total_like'] =  $info['total_like'];
            $item['total_comment'] =  $info['total_comment'];
...
15. tags
16. item_screen - url
17. media_type - oimage / vide
18. item_video - video url
...
function save_post(){
    ...
    $media_type = isset($_REQUEST['media_type']) ? $_REQUEST['media_type'] : null;
    $custom_fields = array();
    ...
    if (isset($_REQUEST['item_video'])) {
        $valid_formats = array("avi", "mp4"); // Valid image formats
        $ext = pathinfo($_REQUEST['item_video'], PATHINFO_EXTENSION);
        if (in_array($ext, $valid_formats)) {
            //Valid File extension check
        } else {
            $errors['message'] = 'Only video allowed';
        }
        $item_video = $_REQUEST['item_video'];
    }
    ...
    $item_insrt->media_type = $media_type;
    $item_insrt->screen_shot = $item_screen;
    $item_insrt->post_video = $item_video;
...
function update_post(){
    ...
    $pdo = ORM::get_db();
    if(empty($item_screen) && empty($item_video))
    {
        $query = "UPDATE `".$config['db']['pre']."product` set 
             ...
             where id = '".$_REQUEST['product_id']."'  LIMIT 1";
    }else if(!empty($item_screen))
    {
        $query = "UPDATE `".$config['db']['pre']."product` set 
             user_id = '".$_REQUEST['user_id']."',
             product_name = '".$_REQUEST['title']."',
    ...
    // $product_id = $item_insrt->id();
    save_post_customField_data($custom_fields,$_REQUEST['product_id']);
    $results['status'] = "success";
...
function userTransactions()
{
  ...
  $rows = ORM::for_table($config['db']['pre'].'transaction')
    ...
    {
        $cat['id'] = $info['id'];
        $cat['product_name'] = $info['product_name'];
        $cat['amount'] = $info['amount'];
        $cat['payment_method'] = $info['transaction_gatway'];
        $cat['status'] = $info['status'];
        $cat['transaction_time'] = $info['transaction_time'];
    ...
    send_json($results);
    die();
    /*$query = "SELECT * FROM `".$config['db']['pre']."transaction` 
WHERE seller_id = $sellerid";
    ...
    if ($total > 0) {
        ...
        foreach ($result as $row) {
            $get_city['cityid'] = $row['id'];
            $get_city['cityname'] = $row['product_name'];
            $cities[] = $get_city;
        }
...
function get_ads_options()
{
    ...
    $query = "SELECT *
FROM `".$config['db']['pre']."options`
WHERE option_id in(121,122,123,124,125)";
    ...
    if ($total > 0) {
        $options = array();
        foreach ($result as $row) {
            $get_options['option_id'] = $row['option_id'];
            $get_options['option_name'] = $row['option_name'];
            $get_options['option_value'] = $row['option_value'];
            $options[] = $get_options;
        }
    ...
    $results['status'] = "error";
    $results['message'] = "No ads option found";
}
...
WHERE c.asciiname like '%$dataString%'
 ...
 LIMIT 100";
    ...
    if ($total > 0) {
        ...
        foreach ($result as $row) {
            $get_city['cityid'] = $row['id'];
            $get_city['cityname'] = $row['asciiname'];
            $get_city['stateid'] = $row['subadmin1_code'];
            $get_city['statename'] = $row['statename'];
            $get_city['citystate'] = $row['asciiname'].', '.$row['statename'];
            $cities[] = $get_city;
        }
    ...
    $results['status'] = "error";
    $results['message'] = "No city found";
}

function add_post_like(){
    global $config,$lang,$results;
    $user_id = isset($_REQUEST['user_id']) ? $_REQUEST['user_id'] : null;
    $product_id = isset($_REQUEST['post_id']) ? $_REQUEST['post_id'] : null;

    $result_num = ORM::for_table($config['db']['pre'].'product_like')
    ->where(array(
        'user_id' => $user_id,
        'product_id' => $product_id
    ))
    ...
    if($result_num>0){
        $q_result = ORM::for_table($config['db']['pre'].'product_like')
        ->where(array(
            'user_id' => $user_id,
            'product_id' => $product_id
        ))
        ...
        $item_edit = ORM::for_table($config['db']['pre'].'product')->find_one($product_id);
        $like_total = $item_edit['total_like'];
        $total_like = $like_total - 1;
        $item_edit->set('total_like', $total_like);
        $item_edit->save();
    }else{
        $item_insrt = ORM::for_table($config['db']['pre'].'product_like')->create();
        $item_insrt->user_id = $user_id;
        $item_insrt->product_id = $product_id;    
        $item_insrt->save();
...
function add_post_comment(){
    ...
    $item_insrt = ORM::for_table($config['db']['pre'].'product_comment')->create();
    $item_insrt->user_id = $user_id;
    $item_insrt->product_id = $product_id;    
    $item_insrt->comment = $comment;    
    $item_insrt->save();

    $item_edit = ORM::for_table($config['db']['pre'].'product')->find_one($product_id);
    $comment_total = $item_edit['total_comment'];
    $total_comment = $comment_total +1;
    $item_edit->set('total_comment', $total_comment);
    $item_edit->save();
    
    $results['status'] = "success";
    $results['message'] = "Comment added Successfully";
    send_json($results);
    die();
}
...
function add_post_reply(){
    ...
    $reply = isset($_REQUEST['reply']) ? $_REQUEST['reply'] : null;
    $user_id = isset($_REQUEST['user_id']) ? $_REQUEST['user_id'] : null;
    // $product_id = isset($_REQUEST['post_id']) ? $_REQUEST['post_id'] : null;

    $item_edit = ORM::for_table($config['db']['pre'].'product_comment')->find_one($comment_id);
    $item_edit->set('reply_id', $user_id);
    $item_edit->set('reply_comment', $reply);
    $item_edit->set('replied_at', date('Y-m-d H-i:s'));
    $item_edit->save();
    
    $results['status'] = "success";
    $results['message'] = "Reply added Successfully";
    send_json($results);
    die();
}
...
function get_comment_list(){
    ...
    $rows = ORM::for_table($config['db']['pre'].'product_comment')
    ...
    foreach ($rows as $info)
    {
        $user = ORM::for_table($config['db']['pre'].'user')
            ...
            ->find_one();   
        $replyuser = ORM::for_table($config['db']['pre'].'user')
            ->select_many('id', 'email', 'username', 'name')
            ->where('id', $info['reply_id'])
            ->find_one();   
        $data['id'] = $info['id'];
        $data['commented_by_id'] = $info['user_id'];
        $data['commented_by_name'] = $user['name'];
        $data['product_id'] = $info['product_id'];
        $data['comment'] = $info['comment'];
        $data['reply_comment'] = $info['reply_comment'];
        $data['replied_by_id'] = $info['reply_id'];
        $data['replied_by_name'] = $replyuser['name'];
        $data['added_at'] = $info['added_at'];
        $data['replied_at'] = $info['replied_at'];
...
function upload_product_video(){
    ...
    $getAvatar = fileUpload($path_avatar, $file_avatar, "video", $first_title, 800, 800,true);

    if($getAvatar != ""){
        $results['status'] = "success";
        $results['video_url'] = $getAvatar;
