...
function get_products_data($userid=null,$cat_id=null,$subcat_id=null,$location=false,$country_code=null,$state_code=null,$city=null,$status=null,$premium=false,$page=null,$limit=null,$order=false,$sort="id",$sort_order="DESC",$item_id=null){
   ...
   if($order){
    ...
    $pdo = ORM::get_db();
if($item_id != null){
    $query = "SELECT p.id,p.product_name,p.featured,p.urgent,p.highlight,p.price,p.category,p.sub_category,p.tag,p.screen_shot,p.user_id,p.city,p.country,p.state,p.status,p.hide,p.created_at,p.expire_date,p.description,p.phone
...
{
    $query = "SELECT p.id,p.product_name,p.featured,p.urgent,p.highlight,p.price,p.category,p.sub_category,p.tag,p.screen_shot,p.user_id,p.city,p.country,p.state,p.status,p.hide,p.created_at,p.expire_date,p.description,p.phone,p.latlong,
