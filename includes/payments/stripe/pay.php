...
if (empty($action) && isset($_SESSION['markad'][$access_token]['payment_type'])) {
...
if ( !empty($action) ) {
    ...
    switch ($action) {
        ...
        case 'stripe_payment_process':
            ...
            $plan_interval = $_SESSION['markad'][$access_token]['plan_interval'];
            $payment_mode = $_SESSION['markad'][$access_token]['payment_mode'];
            ...
            try {
                ...
                $total_price = $amount;
                $markad_currency = $config['currency_code'];
                if ( in_array( $markad_currency, $zero_decimal ) ) {
