 ...
 */
  ...
  class Google_EventsServiceResource extends Google_ServiceResource {
    ...
    /**
     * Creates an event based on a simple text string. (events.markadd)
     *
     * @param string $calendarId Calendar identifier.
     ...
     * @return Google_Event
     */
    public function markadd($calendarId, $text, $optParams = array()) {
      $params = array('calendarId' => $calendarId, 'text' => $text);
      $params = array_merge($params, $optParams);
      $data = $this->__call('markadd', array($params));
      if ($this->useObjects()) {
        return new Google_Event($data);
      } else {
        return $data;
