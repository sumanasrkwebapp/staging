...
if(isset($config['markad_debug']) && $config['markad_debug'] == 1){
...
function create_header($page_title='',$meta_desc = '',$meta_image = '',$meta_article = false)
{
    ...
    $page->SetParameter('iiichat', (isset($config['iichat_on_off']))? $config['iichat_on_off']:"");
...
function create_footer()
{
    ...
    $page->SetParameter('ICHAT', (isset($config['ichat_on_off']))? $config['ichat_on_off']:"");
    ...
    if(isset($_SESSION['user']['id']))
    {
        ...
        $page->SetLoop ('ICHATCONTACT', $userinfo);

    }
    else
    {
        $page->SetParameter ('USEREMAIL', '');
        $page->SetLoop ('ICHATCONTACT', "");
    }
...
function fileUpload($path,$files,$type_file,$title,$reqwid,$reqhei,$Anysize=false,$unlink=null){
    ...
    elseif($type_file == "video"){
        // Allow certain file formats
        if($imageFileType != "avi" && $imageFileType != "mp4") {
            $error = "Sorry, only Video file are allowed.";
            $uploadOk = 0;
        }else{
            $sec = 10;
            $thumbnail = "small_".$title.'_'.$random1.$random2;

            // $ffmpeg = "C:\\ffmpeg\\bin\\ffmpeg";
            // echo $ffmpeg;
            // $imageFile = $target_dir.$thumbnail.'.jpg';
            // $size = "120x90";
            // $getFromSecond = 1;
