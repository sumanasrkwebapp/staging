...
function get_customFields_by_catid($maincatid=null,$subcatid=null,$require=true,$fields=array(),$data=array()){
    ...
    foreach ($result as $info)
    {
        ...
        if($info['custom_type'] == 'text-field'){
            $value = '';
            if(in_array($info['custom_id'], $fields)) {
                $index = array_search($info['custom_id'],$fields,true);
                $value = $data[$index];
            } else {
                $value = $custom_fields[$info['custom_id']]['default'];
            }
            $textbox = '<input name="custom['.$info['custom_id'].']" id="custom['.$info['custom_id'].']" class="form-control"  type="text" value="'.$value.'" '.$required.' placeholder="'.$custom_fields[$info['custom_id']]['title'].'"/>';
...
function payment_success_save_detail($access_token){
    ...
    $folder = $_SESSION['markad'][$access_token]['folder'];
    $payment_type = $_SESSION['markad'][$access_token]['payment_type'];
    $user_id = $_SESSION['user']['id'];
    $now = time();

    if($payment_type == "subscr"){
        $trans_desc = $title;
        $subcription_id = $_SESSION['markad'][$access_token]['sub_id'];
        ...
        if(!empty($subsc_details)){
            ...
            elseif($txn_type == 'subscr_signup')
            {
                ...
                $subscription_stripe_customer_id = isset($_SESSION['markad'][$access_token]['customer_id'])? $_SESSION['markad'][$access_token]['customer_id'] : null;
                $subscription_stripe_subscription_id = isset($_SESSION['markad'][$access_token]['subscription_id'])? $_SESSION['markad'][$access_token]['subscription_id'] : null;
                $subscription_billing_day = isset($_SESSION['markad'][$access_token]['billing_day'])? $_SESSION['markad'][$access_token]['billing_day'] : null;
                $subscription_length = 0;
                $subscription_interval = isset($_SESSION['markad'][$access_token]['interval'])? $_SESSION['markad'][$access_token]['interval'] : null;
                $subscription_trial_days = isset($_SESSION['markad'][$access_token]['trial_days'])? $_SESSION['markad'][$access_token]['trial_days'] : null;
                $subscription_date_trial_ends = isset($_SESSION['markad'][$access_token]['date_trial_ends'])? $_SESSION['markad'][$access_token]['date_trial_ends'] : null;
            ...
            unset($_SESSION['markad'][$access_token]);
        ...
        $item_pro_id = $_SESSION['markad'][$access_token]['product_id'];
        $item_featured = ($_SESSION['markad'][$access_token]['featured'] == "1") ? "1" : "0";
        $item_urgent =  ($_SESSION['markad'][$access_token]['urgent'] == "1") ? "1" : "0";
        $item_highlight = ($_SESSION['markad'][$access_token]['highlight'] == "1") ? "1" : "0";
        $trans_desc = $_SESSION['markad'][$access_token]['trans_desc'];
...
function payment_fail_save_detail($access_token){
    ...
    if($payment_type == "subscr"){
        ...
        $item_pro_id = $_SESSION['markad'][$access_token]['product_id'];
        $item_featured = $_SESSION['markad'][$access_token]['featured'];
        $item_urgent = $_SESSION['markad'][$access_token]['urgent'];
        $item_highlight = $_SESSION['markad'][$access_token]['highlight'];
        $trans_desc = $_SESSION['markad'][$access_token]['trans_desc'];
...
function payment_error($status,$error_message="",$access_token){
    ...
    {
        if(isset($_SESSION['markad'][$access_token]['transaction_id']))
        {
            $transaction_id = $_SESSION['markad'][$access_token]['transaction_id'];
            unset($_SESSION['markad'][$access_token]);
