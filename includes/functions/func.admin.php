...
function check_purchse_valid(){
    ...
    if((time()-$cron_validation_exec_time) > $cron_validation_time) {
        ...
        if(isset($config['purchase_key'])){
            $url = "https://canders.in/api/api.php?verify-purchase=" . $config['purchase_key'] . "&site_url=". $config['site_url'];
            $ch = curl_init();
            ...
            if ($output['success']) {
                ...
                unlink($filename);
                delete_option("markad_secret_file");
                delete_option("purchase_key");
                $status = "error";
                $message = $output['error'];
            }
        }else{
            if(isset($config['markad_secret_file'])){
                $filename = "admin/".$config['markad_secret_file'];
                unlink($filename);
                delete_option("markad_secret_file");
                delete_option("purchase_key");
                $status = "error";
